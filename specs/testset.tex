...
The transfer slack for the transfer is defined, or can be reduced to zero if enough time is available to make the transfer.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[] (slack) [above=1.6cm of 3] {transfer slack: 0s};
\end{tikzpicture}
\end{minipage}
\label{fig:singlemodality_network}
\vspace{-2em}
...
\subsubsection*{Synthetic Tests}
\begin{enumerate}
\item \textbf{given} a default transfer slack of 60 seconds and a specified transfer slack of 0 seconds at stop 3, \textbf{depart} at 00:01 from stop 1 heading to stop 4.
\end{enumerate}
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[] (17) [left=.2cm of 1b] {\tiny KV17 cancel / Trip Banning};
